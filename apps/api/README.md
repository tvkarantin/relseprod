...
- **Этап 3** — библиотека рилсов: список с поиском и серверной пагинацией,
  карточка рилса, сохранение пользовательского сценария, счётчики дашборда и
  раздел «Мои рилсы». Подключён React-frontend `apps/web`.
- **Этап 4** — интеграция с Deepgram для точной транскрибации речи из видео.
- **Этап 5** — интеграция с OpenRouter (AI) для перевода и структурного разбора речи (хук, основная часть, CTA).
...
| `GET` | `/api/v1/reels` | библиотека: `competitor_id`, `search`, `page`, `limit` |
| `GET` | `/api/v1/reels/my` | рилсы в работе: `content_status`, `search`, `page`, `limit` |
| `GET` | `/api/v1/reels/{id}` | один рилс вместе с конкурентом и сценарием |
| `PUT` | `/api/v1/reels/{id}/content` | сохранить сценарий |
| `GET` | `/api/v1/dashboard/summary` | реальные счётчики по базе |
...
Параметры: `competitor_id` (> 0), `search` (до 200 символов), `page` (≥ 1),
`limit` (1–100, по умолчанию 20).

Порядок **фиксированный**: `published_at DESC` (записи без даты в конце), затем
`created_at DESC`, затем `id DESC`. Параметра `sort` нет.

Ответ содержит `items`, `page`, `limit`, `total`, `pages`. Страница за пределами
последней возвращает пустой `items` и код 200, а не ошибку. Конкурент и сценарий
загружаются через `joinedload`, поэтому запрос не порождает N+1.

### Поиск

Регистронезависимый поиск по `reels.caption`, `competitor.instagram_username`,
а также `hook`, `script`, `cta` и `notes` из `reel_content`.

Особенности реализации:

- пользовательские `%` и `_` экранируются и ищутся буквально;
- пустой или состоящий из пробелов `search` считается отсутствующим;
- значение обрезается по краям перед поиском;
- встроенный `LOWER()` в SQLite работает только с ASCII, поэтому на каждом
  соединении регистрируется Unicode-версия функции — иначе поиск по кириллице
  не находил бы «Маркетинг» по запросу «МАРКЕТИНГ».

### `PUT /api/v1/reels/{id}/content`

Принимает полный объект редактора:

```json
{
  "hook": "Почему ваши Reels не набирают просмотры?",
  "script": "Текст сценария",
  "cta": "Напишите «РАЗБОР» в комментариях",
  "notes": "В начале добавить резкий переход",
  "contentStatus": "script"
}
```

Ограничения длины: `hook` — 500, `script` — 10000, `cta` — 1000, `notes` — 10000.
Допустимые статусы: `new`, `idea`, `script`, `ready`, `published`, `archived`.

Текст сохраняется дословно: переносы строк и отступы не удаляются, пустые строки
допустимы. Поля всегда строки — `null` не используется. Данные Instagram этим
эндпоинтом не изменяются.

Если у рилса ещё нет строки `reel_content` (например, он импортирован ранней
версией), она создаётся автоматически при чтении и при сохранении.
...
│   │       ├── reels.py         # библиотека рилсов и редактор
│   │       └── dashboard.py     # счётчики
│   ├── core/
...
│   │   └── reel_content.py      # библиотека, редактор, дашборд
│   └── tasks/
